...
- **category loss** :
  ...
  grtr_category = tf.cast(features[..., 5], tf.int32)
  ...
  >  tensor의 data type을 그간 계속 tf.float32를 사용했는데, grtr_category를 tf.int32로 casting하지 않고 tf.losses.sparse_categorical_crossentropy를 사용하면 이상한 값이 출력됐다. tensorflow 튜토리얼에서 딱히 data type에 대한 언급은 없었지만 예제에도 정수를 사용하였다.
